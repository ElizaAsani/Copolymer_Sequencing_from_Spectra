"""
Code to probabilistically decode sequences from spectra
"""
    ...
    for _, data in enumerate(test_dl):
        ...
        with torch.no_grad():
            while input_len < max_len and not finished.all():
                ...
                decoder_mask_subset = padding_mask & causal_mask.unsqueeze(0).unsqueeze(0)
...
def _batched_beam_search(model, encoder_input, vocab_size, batch_size, n_steps, beam_width, alpha, device='cpu'):
    ...
    for i in range(n_steps):
        ...
        causal_mask = SeqDataset.causal_mask(input_len).to(device)
        padding_mask = (decoder_input != pad_token).unsqueeze(1).unsqueeze(1)
        decoder_mask_subset = padding_mask & causal_mask.unsqueeze(0).unsqueeze(0)
